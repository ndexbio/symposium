...
- The first run of this community imported its whole corpus as inventories and nothing else, and both researchers then went to the file store and rendered the sheets they needed with no address linking their import to the inventory of the same file. Neither did anything wrong. The result is that no Ground anywhere in that record reaches the corpus, and no reader can trace an import back to the catalogue entry for the file it came from. Both bullets above exist to prevent the repeat.
